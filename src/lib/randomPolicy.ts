...
// ── Sample data pools ────────────────────────────────────────
const firstNames = ['James','Maria','Robert','Linda','Michael','Patricia','David','Jennifer','William','Barbara','Richard','Susan','Joseph','Jessica','Thomas','Sarah','Charles','Karen','Christopher','Lisa','Daniel','Nancy','Matthew','Betty','Anthony','Margaret','Mark','Sandra','Donald','Ashley','Steven','Dorothy','Paul','Kimberly','Andrew','Emily','Kenneth','Donna','Joshua','Michelle','Kevin','Carol','Brian','Amanda','George','Melissa','Edward','Deborah','Ronald','Stephanie'];

const lastNames = ['Smith','Johnson','Williams','Brown','Jones','Garcia','Miller','Davis','Martinez','Hernandez','Lopez','Gonzalez','Wilson','Anderson','Taylor','Thomas','Jackson','White','Harris','Sanchez','Clark','Ramirez','Lewis','Robinson','Walker','Perez','Hall','Young','Allen','King','Wright','Scott','Torres','Nguyen','Hill','Flores','Green','Adams','Nelson','Baker','Mitchell','Carter','Roberts','Turner','Phillips','Campbell','Parker','Evans','Collins','Edwards'];

const streets = ['Oak','Main','Maple','Cedar','Pine','Elm','Washington','Lake','Hill','Park','Sunset','Willow','River','Highland','Meadow','Forest','Valley','Ridge','Spring','Church'];
const streetTypes = ['St','Ave','Blvd','Dr','Ln','Rd','Way','Ct','Pl','Cir'];

const cities: Record<string, string[]> = {
  CA: ['Los Angeles','San Diego','San Jose','Sacramento','Fresno','Long Beach','Oakland','Bakersfield','Anaheim','Riverside'],
  TX: ['Houston','San Antonio','Dallas','Austin','Fort Worth','El Paso','Arlington','Corpus Christi','Plano','Lubbock'],
  FL: ['Jacksonville','Miami','Tampa','Orlando','St. Petersburg','Hialeah','Tallahassee','Fort Lauderdale','Port St. Lucie','Pensacola'],
...
const colors = ['White','Black','Silver','Gray','Red','Blue','Dark Blue','Pearl White','Charcoal','Burgundy','Green','Bronze'];

const agentNames = ['Rivera Insurance Agency','Coastal Auto Insurance','Summit Coverage Group','Horizon Insurance Services','Alliance Auto Group','Premier Insurance Agency','Eagle Insurance Partners','Liberty Coverage Center','Keystone Insurance Group','Apex Auto Insurance'];
...
function generateAgentCode(agencyId: string): string {
  switch (agencyId) {
    case 'farmers':     return `${pad(rand(1, 99), 2)}-${pad(rand(100, 999), 3)}`;
    case 'statefarm':   return pad(rand(10000000, 99999999), 8);
    case 'allstate':    return `R${pad(rand(100000, 999999), 6)}`;
    case 'progressive': return `AG${pad(rand(10000, 99999), 5)}`;
    case 'geico':       return `GC${pad(rand(1000, 9999), 4)}`;
    case 'libertymutual': return `LM${pad(rand(100000, 999999), 6)}`;
    case 'nationwide':  return `NW${pad(rand(10000, 99999), 5)}`;
    case 'usaa':        return `UA${pad(rand(10000, 99999), 5)}`;
    default:            return `AGT-${pad(rand(1000, 9999), 4)}`;
  }
}
...
export function generateRandomPolicy(agency: Agency) {
  ...
  const insuredZip   = localAgent ? pick(['57701','57702','57703','57709']) : zip;
  ...
  const collisionDed  = hasFullCoverage ? pick(['$250','$500','$500','$1,000','$1,000','$2,000']) : '';
  const comprehensiveDed = hasFullCoverage ? pick(['$100','$250','$500','$500','$1,000']) : '';
  const rental = hasFullCoverage ? pick(['','$30/day (max $900)','$40/day (max $1,200)','$50/day (max $1,500)']) : '';
  const roadside = pick(['','','Included — Basic','Included — Basic','Included — Premium']);
  ...
  const downPayment = (Math.ceil((basePremium + veh2Premium) * 0.2 / 5) * 5).toFixed(2);

  const dob = new Date(rand(1955, 2000), rand(0, 11), rand(1, 28)).toISOString().split('T')[0];
  ...
  return {
    ...
    paymentPlan:      pick(['Monthly','Monthly','Monthly','Quarterly','Semi-Annual','Paid in Full']),
    billingSchedule:  pick(['Monthly — 1st','Monthly — 5th','Monthly — 15th','Quarterly — 1st']),
